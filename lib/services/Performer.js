...
exports.process = async function (req, res, action, bodyObj) {
	...
	try {
		...
		// add 'delay' to emulate some api is slow	
		const delay = result && result.resOptions && result.resOptions.delay
		if(delay) {
			await wait(delay);
		}
	} catch (reason) { err = reason }

	const ignor = !result || (result && result.resOptions && result.resOptions.ignor) || (err && err.ignor)
	if(ignor) {
		self.logger.restlog( err, 'Result is ignored', purify( { result: result }, null, 0, []), 'info' )
		throw {
			ignor : true
		}
	}
